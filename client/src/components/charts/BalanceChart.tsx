      ...
      <svg viewBox={`0 0 ${W} ${H}`} width="100%" style={{ display: 'block', overflow: 'visible' }}
        ...
        <defs>
          <linearGradient id="balGrad" x1="0" y1="0" x2="0" y2="1">
            <stop offset="0%" stopColor={`var(${accentVar})`} stopOpacity="0.20" />
        ...
        {realPts.length > 0 && <path d={line(realPts)} fill="none" stroke={`var(${accentVar})`} strokeWidth="2.4" strokeLinejoin="round" strokeLinecap="round" />}
        {fullProj.length > 0 && <path d={line(fullProj)} fill="none" stroke={`var(${accentVar})`} strokeWidth="2.2" strokeDasharray="2 5" strokeLinecap="round" opacity="0.7" />}
        ...
        {hover != null && (
          <g>
            ...
            <circle cx={x(hover)} cy={y(all[hover].value)} r="5" fill="var(--surface)" stroke={`var(${accentVar})`} strokeWidth="2.4" />
          </g>
        )}
      </svg>
      {hover != null && (
        <div style={{
          ...
          <div className="tnum">{fmtEur(all[hover].value)}</div>
        </div>
      )}
    </div>
  );
